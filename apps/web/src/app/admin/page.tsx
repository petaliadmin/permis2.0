    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        {statCards.map((c, i) => (
          ...
          >
            <i className={`ti ${c.icon} text-xl ${c.color}`} aria-hidden="true" />
            <p className="mt-2 font-display text-2xl font-black text-foreground">
              {c.value ?? '—'}
            </p>
            <p className="text-xs font-medium leading-tight text-muted">{c.label}</p>
          </motion.div>
        ))}
      </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <div className="rounded-2xl border border-token bg-surface-1 p-5 shadow-soft">
              ...
              <div className="flex items-center justify-between border-b border-token py-2 text-sm">
                ...
                <span className="font-bold text-red-500">−{fmtXof(rev.costs.sms.totalXof)}</span>
              </div>
              <div className="flex items-center justify-between border-b border-token py-2 text-sm">
                <span className="text-foreground">
