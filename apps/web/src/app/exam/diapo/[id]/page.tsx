...
interface DiapoQuestion {
  q: number;
  image: string | null;
  enonce: string;
  propositions: DiapoProposition[];
  answer: string[];
}
interface DiapoExam {
  id: number;
  title: string;
  questions: DiapoQuestion[];
}
    ...
    return (
      <div className="on-ink flex min-h-screen flex-col bg-ink px-5 pt-[calc(env(safe-area-inset-top)+28px)] pb-8 text-center">
        ...
        >
          ...
          <p
            className={`mt-6 font-display text-xl font-black ${passed ? 'text-orange-400' : 'text-danger-400'}`}
          >
          ...
          <p className="mt-1 text-sm text-slate-400">
            {passed
              ...
              : `Il faut ≥ 70 % (${Math.ceil(total * 0.7)} bonnes réponses).`}
          </p>
          ...
          >
            ...
            <div className="flex flex-col gap-3">
              {q.propositions.map((prop) => {
                ...
                if (isConfirmed) {
                  if (isCorrect) {
                    cls = 'border-success-500 bg-success-50 text-success-700';
                    badge = 'bg-success-600 text-white';
                  } else if (isSel) {
                    cls = 'border-danger-500 bg-red-50 text-danger-600';
        ...
        <div className="mx-auto flex max-w-lg gap-3">
          ...
          {!isConfirmed ? (
            <button
              ...
              className="btn-orange flex-1 disabled:opacity-40"
            >
              Confirmer
            </button>
          ) : (
            <button onClick={goNext} className="btn-orange flex-1">
              {currentQ < total - 1 ? 'Suivante' : 'Résultats 🏁'}
            </button>
          )}
        </div>
      </div>
