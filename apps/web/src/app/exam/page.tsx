...
export default function ExamPage() {
  ...
  useEffect(() => {
    loadData<{ exams: DiapoMeta[] }>('/exams/diapos', '/data/diapos.json')
      .then((d) => setExams(d.exams))
      .catch(console.error)
      .finally(() => setLoading(false));
  }, []);
  // EntitlementProvider in layout already calls fetchEntitlements; this is a safety net
  useEffect(() => {
    if (isAuthenticated && !entitlementsReady) fetchEntitlements();
  }, [isAuthenticated, entitlementsReady, fetchEntitlements]);
  ...
  return (
    <AppShell>
      <PageHeader
        title="Examens"
        subtitle="Séries d’entraînement conformes à l’examen"
        accent="orange"
        menu
      >
        {/* Tabs */}
          ...
          <div className="space-y-3">
            {filtered.map((exam, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`relative flex h-20 w-24 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br ${THUMB_GRAD[i % THUMB_GRAD.length]}`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      {isFree ? (
                        <span className="chip chip-success">Gratuit</span>
                      ) : !unlocked ? (
                        <span className="chip chip-orange">Premium</span>
                      ) : (
